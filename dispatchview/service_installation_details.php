...
if(isset($_POST["submit"]))
 {
	...
	elseif($branch != "" && $installer_name== "" && $client_name != "")
	{
		if($mode=="Close")
	 	{
			$query_inst = mysql_query("select installation.id,req_date,request_by,company_name,user_id,inst_id,inst_name,rtime,re_city_spr_1.name as zonearea, time, no_of_vehicals,installation_made,model from installation left join re_city_spr_1   on installation.Zone_area = re_city_spr_1.id where time>='". $from_date." 00:00:00"."' and time<='".$to_date." 23:59:59"."' and (installation_status=5 or installation_status=15) and (branch_id='".$branch."' or inter_branch='".$branch."') and user_id='".$client_name."'");
	...
	elseif($branch != "" && $installer_name== "" && $client_name == "")
	{
		if($mode=="Close")
	 	{
			$query_inst = mysql_query("select installation.id,req_date,request_by,company_name,user_id,inst_id,inst_name,rtime,re_city_spr_1.name as zonearea, time, no_of_vehicals,installation_made,model from installation left join re_city_spr_1   on installation.Zone_area = re_city_spr_1.id where time>='". $from_date." 00:00:00"."' and time<='".$to_date." 23:59:59"."' and (installation_status=5 or installation_status=15) and (branch_id='".$branch."' or inter_branch='".$branch."')");
 ...
 {	 
	...
	$count_total =  $count1 + $count2;
	 ?>
	 	 ...
	 	 <a href="service_installation_details.php?zone_id=<?=$row["id"];?>&from=<?=$from_date;?>&to=<?=$to_date;?>">
        ...
        <td>
        	<table>
            	<tr>
                    ...
                    <!--<td>Branch</td>
                    <td><select name="branch" id="branch_id">
                ...
                <tr>
                    ...
                    <td colspan="3">
                     ...
                     <select name="main_user_id" id="main_user_id">
                            ...
                            <?php 
                            $main_user_iddata=mysql_query("SELECT Userid AS user_id,UserName AS name FROM addclient WHERE Branch_id='".$_SESSION['BranchId']."'");
...
<?
unlink(__DOCUMENT_ROOT.'/dispatchview/reportfiles/service_installation_excel.xls');
$filepointer=fopen(__DOCUMENT_ROOT.'/dispatchview/reportfiles/service_installation_excel.xls','w');
