...
if($_GET["edit"]==true && $_GET["rowid"]!='')
{
	...
	$sql = mysql_query("SELECT Userid AS id,UserName AS sys_username FROM addclient WHERE Userid='$main_user_id'",$dblink2);
	...
	{*/
		if($atime_status=="Till")
		{
			...
			//1-New,2-assigned,3-newbacktoservice,4-backtoservice,5-close,6-callingclose
			   $sql="INSERT INTO `services` (`req_date`, `request_by`, `name`, `user_id`, `company_name`, `veh_reg`, `device_imei`, 
			  `date_of_installation`,  `Notwoking`, `location`, `atime`, `pname`,atime_status, `cnumber`,`device_model`, `comment`,
			  `required`,`IP_Box`,branch_id,service_status,Zone_area,service_reinstall,`inter_branch`,branch_type) 
			  VALUES ('".$date."','".$account_manager."', '".$username."', '".$main_user_id."', '".$company."', '".$veh_reg."',
			  '".$TxtDeviceIMEI."','".$date_of_install."','".$Notwoking."', '".$location."', '".$time."','".$pname."','".$atime_status."', 
			  '".$cnumber."', '".$Device_model."', '".$comment."','".$required."','".$IP_Box."','1',1,'".$Zone_area."',
			  '".$service_reinstall."','0','".$branch_type."');";
			
			
			$execute=mysql_query($sql,$dblink2);// or die(mysql_error());
			header("location:notworkingvehicle.php");
		}
		if($atime_status=="Between")
		{
			$time=$_POST['time1'];
			...
			//1-New,2-assigned,3-newbacktoservice,4-backtoservice,5-close,6-callingclose
			  $sql="INSERT INTO `services` (`req_date`, `request_by`, `name`, `user_id`, `company_name`, `veh_reg`, `device_imei`, 
			`date_of_installation`,  `Notwoking`, `location`, `atime`, `atimeto`, `pname`,atime_status, `cnumber`,`device_model`, 
			`comment`,`required`,`IP_Box`,branch_id,service_status,Zone_area,service_reinstall,`inter_branch`,branch_type) 
			VALUES ('".$date."','".$account_manager."', '".$username."', '".$main_user_id."', '".$company."', '".$veh_reg."',
			'".$TxtDeviceIMEI."','".$date_of_install."','".$Notwoking."', '".$location."', '".$time."','".$totime."','".$pname."',
			'".$atime_status."', '".$cnumber."', '".$Device_model."', '".$comment."','".$required."','".$IP_Box."','1',1,
			'".$Zone_area."','".$service_reinstall."','0','".$branch_type."');";
 ...
 <form name="myForm" action="" onsubmit="return validateForm()" method="post">
   <table style="width: 900px;" cellspacing="2" cellpadding="3" border="1">
		<tr>
            ...
            <td align="center"> 
            <?php
            $main_user_iddata = mysql_query("SELECT Userid AS user_id,UserName AS `name` FROM addclient WHERE Userid='".$main_user_id."'",$dblink2);
            $data = mysql_fetch_assoc($main_user_iddata);
            
            ?>
           <!-- <input type="hidden" name="main_user_id" id="TxtMainUserId" value="<?php echo $data['user_id']; ?>" readonly="readonly" />
            <input type="text" name="user_name" id="TxtMainUserId" value="<?php echo $data['name']; ?>" readonly="readonly" />-->
            <label><strong><?php echo $data['name'];?></strong> </label>
            </td>
            
            <td  align="right"><strong>Company Name*:</strong></td>
            <td align="center"><!--<input type="text" name="company" id="TxtCompany" readonly value="<?echo $company?>"/>-->
            	<label><strong><?php echo $company;?></strong> </label></td>
...
<!--    <tr>
    ...
    <tr>
         ...
         &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="Cancel" value="Cancel" onClick="window.location = 'notworkingvehicle.php' " /> </td>
    </tr>
