	...
	if($action=='edit')
		...
		if($atime_status=="Between")
		 {
			...
			$sql="update installation set sales_person='".$sales_person."', `user_id`= '".$main_user_id."', `company_name`='".$company."',no_of_vehicals='".$no_of_vehicals."',location='".$location."',model='".$model."',time='".$time."',totime='".$totime."',atime_status='".$atime_status."',contact_number='".$cnumber."' ,contact_person='".$contact_person."',dimts='".$dimts."',demo='".$demo."',veh_type='".$veh_type."',comment='".$comment."',immobilizer_type='".$immobilizer_type."',payment_req='".$payment_req."',required='".$required."',IP_Box='".$IP_Box."',Zone_area='".$Area."' ".$inst_status.",`inter_branch`='".$city."',branch_type='".$branch_type."',instal_reinstall='".$instal_reinstall."' where id=$id";
	   ...
	   else if($action=='editp')
		{
			...
			if(isset($_GET['pending']))
			{
				...
				if($_SESSION['BranchId']==1 && $branch_type == "Samebranch" && $back_reason=="" && $instal_reinstall == "installation")
				{
					$setqry=", status='1' , installation_status=8";
				}
				elseif($branch_type == "Interbranch" && $_POST['inter_branch_loc']==1 && $back_reason=="" && $instal_reinstall == "installation")
				{
					$setqry=", status='1' , installation_status=8";
					...
					$setqry=", status='1' , installation_status=1";
				}
			}
		if($atime_status=="Till")
		{
			...
			$sql="update installation set sales_person='".$sales_person."', `user_id`= '".$main_user_id."', `company_name`='".$company."',no_of_vehicals='".$no_of_vehicals."',location='".$location."',model='".$model."',time='".$time."',atime_status='".$atime_status."',contact_number='".$cnumber."' ,contact_person='".$contact_person."',dimts='".$dimts."',demo='".$demo."',veh_type='".$veh_type."',comment='".$comment."',immobilizer_type='".$immobilizer_type."',payment_req='".$payment_req."',required='".$required."',IP_Box='".$IP_Box."',`inter_branch`='".$city."' ,Zone_area='".$Area."' ".$setqry.",branch_type='".$branch_type."',instal_reinstall='".$instal_reinstall."' where id=$id";	
		
			$execute=mysql_query($sql);
			header("location:pendinginstallation.php?for=formatrequest");
		  }
	  ...
	  else
	  {
		if($atime_status=="Till")
		{
			$time=$_POST['time'];
				...
				//1-New,2-assigned,3-newbacktoservice,4-backtoservice,5-close,6-callingclose
		   $sql="INSERT INTO installation(`req_date`, `request_by`,sales_person,`user_id`, `company_name`, no_of_vehicals, location,model,time, contact_number,installed_date, status, contact_person, dimts,demo, veh_type,comment, immobilizer_type, payment_req,required, IP_Box,branch_id,installation_status, Zone_area,atime_status,`inter_branch`, branch_type, instal_reinstall) VALUES('".$date."','".$account_manager."','".$sales_person."', '".$main_user_id."', '".$company."','".$no_of_vehicals."','".$location."','".$model."','".$time."','".$cnumber."',now(),1,'".$contact_person."','".$dimts."','".$demo."','".$veh_type."','".$comment."','".$immobilizer_type."','".$payment_req."','".$required."','".$IP_Box."','".$_SESSION['BranchId']."','".$installation_status."','".$Area."','".$atime_status."','".$city."','".$branch_type."','".$instal_reinstall."')";
...
<form method="post" action="" name="form1" onSubmit="return req_info();">
 ...
 <tr>
      ...
      <div id="sub4" <? if($result['immobilizer_type']==''){?>style="display:none"<?}?>>
        <table width="60%" border="0" cellpadding="0" cellspacing="" align="left">
        ...
        <table width="60%"  style="padding-left: 75px;" border="0" cellpadding="0" cellspacing="" align="left">
            <tr>
        ...
        <div id="sub3" <? if($result['payment_req']==''){?>style="display:none"<?}?>>
        <table width="100%" border="0" cellpadding="0" cellspacing="" align="left">
            <tr><td align="right"><?php
            echo $msg;
            ?></td></tr>
            <tr>
