...
if($action=='edit' or $action=='editp')
    {
        $result = select_query("select * from internalsoftware.deletion where id=$id");    
    }
?> 
   ...
   if($action=='edit')
    {
         ...
         $query = "update deletion set sales_manager='".$sales_manager."',client='".$client."',user_id='".$user_id."',device_model='".trim($device_model)."',reg_no='".$veh_reg_edit."',device_sim_no='".$Devicemobile."',imei='".trim($device_imei)."',date_of_installation='".$date_of_install."',vehicle_location='".$Txt_location."',device_status='".$device_status."',deactivation_of_sim='".$DeactiveSim."',deletion_date='".$Txtrdd_date."',reason='".$TxtReason."',service_comment='".$result[0]["service_comment"]."<br/>".date("Y-m-d H:i:s")." - " .$service_comment."',delete_veh_status=1 where id=$id";
		...
		$duplicate_chk = select_query("select * from internalsoftware.deletion where user_id='".$user_id."' and reg_no='".$veh_reg."' and 
						 imei='".trim($device_imei)."' and (approve_status='0' || approve_status='1') and final_status='0' and 
						 close_date is null order by id desc");
		
		if(count($duplicate_chk)>0)
		{
			$errorMsg = "This client same vehicel Request Already Initiated. Kindly check!!";
		}
		else
		{
 ...
 <form name="myForm" action="" onsubmit="return validateForm()" method="post">
   ...
   <table style="padding-left:100px;width:500px;" cellspacing="5" cellpadding="5">
        ...
        <tr>
         ...
         <tr>
            ...
            <option value="" >-- select one --</option>
              <?php
        $sales_manager = select_query("SELECT name FROM internalsoftware.sales_person where active=1 and branch_id=".$_SESSION['BranchId']);
        //while ($data=mysql_fetch_assoc($sales_manager))
		for($sl=0;$sl<count($sales_manager);$sl++)
        {
        ?>
        
        <option name="sales_manager" value="<?=$sales_manager[$sl]['name']?>" <? if($result[0]['sales_manager']==$sales_manager[$sl]['name']) {?> selected="selected" <? } ?> > <?php echo $sales_manager[$sl]['name']; ?>
        </option>
        <?php 
        } 
        ?>
        ...
        <tr>
            ...
            <option value="">-- Select One --</option>
            <?php
            $main_user_id = select_query("SELECT Userid AS user_id,UserName AS `name` FROM internalsoftware.addclient WHERE 
							Branch_id=".$_SESSION['BranchId']." and sys_active='1' order by name asc");
            //while ($data=mysql_fetch_assoc($main_user_id))
			for($ur=0;$ur<count($main_user_id);$ur++)
            {
            ?>
            
           <option name="main_user_id" value="<?=$main_user_id[$ur]['user_id']?>" <? if($result[0]['user_id']==$main_user_id[$ur]['user_id']) {?> selected="selected" <? } ?> >
            <?php echo $main_user_id[$ur]['name']; ?>
            </option>
            <?php 
            } 
        ...
        <tr>
            ...
            <?php
            $main_user_device = select_query("SELECT * FROM internalsoftware.device_type");
            //while ($data=mysql_fetch_assoc($main_user_id))
			for($dv=0;$dv<count($main_user_device);$dv++)
            {
            ?>
            
            <option value ="<?php echo $main_user_device[$dv]['device_type'] ?>" <? if($result[0]['device_model']==$main_user_device[$dv]['device_type']) {?> selected="selected" <? } ?> > 
                <?php echo $main_user_device[$dv]['device_type']; ?>    
            </option>
            <?php 
            } 
        ...
        <tr>
            ...
            <td>
            <input type="text" name="TxtDeviceIMEI" id="TxtDeviceIMEI" value="<?=$result[0]['imei']?>" readonly/></td>
        </tr>
        ...
        <tr>
            ...
            <td>
            <input type="text" name="Devicemobile" id="Devicemobile" value="<?=$result[0]['device_sim_no']?>" readonly/></td>
        </tr>
        ...
        <tr>
            ...
            <td>
            <input type="text" name="date_of_install" id="date_of_install" value="<?=$result[0]['date_of_installation']?>" readonly/></td>
        </tr> 
        ...
        <tr><td> <label  id="lbDlDate">Device Status</label></td>
            <td> <select name="device_status" id="device_status" onchange="DeviceCase(this.value)">
                    <option value="">-- Select One --</option>
                    <option value="Sold Vehicle"<? if($result[0]['device_status']=="Sold Vehicle"){?> selected="selected" <? }?>>Sold Vehicle</option>
                    <option value="Vehicle Stand For Long Time"<? if($result[0]['device_status']=="Vehicle Stand For Long Time"){?> selected="selected" <? }?>>
                    Vehicle Stand For Long Time</option>
                    <option value="Stop GPS"<? if($result[0]['device_status']=="Stop GPS"){?> selected="selected" <? }?>>Stop GPS</option>
                    <option value="Device Lost"<? if($result[0]['device_status']=="Device Lost"){?> selected="selected" <? }?>>Device Lost</option>
                    <option value="Device Dead"<? if($result[0]['device_status']=="Device Dead"){?> selected="selected" <? }?>>Device Dead</option>
                    
                    <option value="Physical Damage"<? if($result[0]['device_status']=="Physical Damage"){?> selected="selected" <? }?>>Physical Damage</option>
                    <option value="Vehicle Theft"<? if($result[0]['device_status']=="Vehicle Theft"){?> selected="selected" <? }?>>Vehicle Theft</option>
                    <option value="Sold Vehicle With Device"<? if($result[0]['device_status']=="Sold Vehicle With Device"){?> selected="selected" <? }?>>
                    Sold Vehicle With Device</option>
                    <option value="Buy Back"<? if($result[0]['device_status']=="Buy Back"){?> selected="selected" <? }?>>Buy Back</option>
                    <option value="Other(Device Not In Existence)"<? if($result[0]['device_status']=="Other(Device Not In Existence)"){?> selected="selected" <? }?>>
                    Other(Device Not In Existence)</option> 
                    <option value="Device Removed Against Non-Payment"<? if($result[0]['device_status']=="Device Removed Against Non-Payment"){?> selected="selected" <? }?>>Device Removed Against Non-Payment</option>
                    <option value="Account Deactivate Against Non-Payment"<? if($result[0]['device_status']=="Account Deactivate Against Non-Payment"){?> selected="selected" <? }?>>Account Deactivate Against Non-Payment</option>  
                </select>
            </td>
        </tr>
        <!--<tr><td> <label  id="lbDlDate">Location</label></td>
            <td> <select name="Txt_location" id="Txt_location">
                    <option value="">-- Select One --</option>
                    <option value="gtrack office"<? if($result[0]['vehicle_location']=="gtrack office"){?> selected="selected" <? }?>>Gtrack office</option>
                    <option value="client office"<? if($result[0]['vehicle_location']=="client office"){?> selected="selected" <? }?>>Client Office</option>
                    <option value="client vehicle"<? if($result[0]['vehicle_location']=="client vehicle"){?> selected="selected" <? }?>>Client Vehicle</option>
                    <option value="device lost"<? if($result[0]['vehicle_location']=="device lost"){?> selected="selected" <? }?>>Device Lost</option>
                </select>
            </td>
        </tr>-->
        <tr>
            ...
            <?php //if($action=='edit') {?>
                   <!--<table id="showlocation"  style="width: 300px;"  cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td ><?=$result[0]['vehicle_location']?></td>
                     </tr>
                 </table>-->
            <? //} ?>
        ...
        <td> <input type="text" name="Txt_contactperson" id="Txt_contactperson" value="<?=$result[0]['Contact_person']?>" />
        </td>
        </tr>-->
        ...
        <tr><td> <label  id="lblReason">Deactivation of SIM</label></td>
            <td> 
            <Input type = 'Radio' Name ='DeactiveSim' id="DeactiveSim" value= 'Yes'<?php if($result[0]['deactivation_of_sim']=="Yes"){echo "checked=\"checked\""; }?>
            <?PHP //print $male_status; ?>
            >Yes
            
            <Input type = 'Radio' Name ='DeactiveSim' id="DeactiveSim" value= 'No'<?php if($result[0]['deactivation_of_sim']=="No"){echo "checked=\"checked\""; }?>
        ...
        <tr><td> <label  id="lbDlDate">Deletion date </label></td>
            <td> <input type="text" name="rdd_date" id="datepicker" value="<?=$result[0]['deletion_date']?>" /></td>
            </tr>
...
<script>DeviceCase12("<?=$result[0]['device_status'];?>");</script>