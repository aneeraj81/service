...
?>
        ...
        <td><?  if($query[$i]["reason"]=='Temperory no to Permanent no' || $query[$i]["reason"]=='Personal no to Commercial no' || $query[$i]["reason"]=='Commercial no to Personal no' || $query[$i]["reason"]=='For Warranty Renuwal Purpose')
    {
        if($query[$i]["vehicle_status"]==2 || ($query[$i]["support_comment"]!="" && $query[$i]["service_comment"]==""))
        {echo "Reply Pending at Request Side";}
        elseif($query[$i]["vehicle_status"]==1 && $query[$i]["final_status"]!=1){echo "Pending at Tech Support Team";}
        elseif($query[$i]["final_status"]==1){echo "Process Done";}
    }
    else{
        if($query[$i]["vehicle_status"]==2 || (($query[$i]["support_comment"]!="" || $query[$i]["admin_comment"]!="") && $query[$i]["service_comment"]==""))
        {echo "Reply Pending at Request Side";}
        elseif($query[$i]["new_reg_no"]=="" && $query[$i]["reason"]=="" && $query[$i]["approve_status"]==0){echo "Request Not Completely Generate.";}
        elseif($query[$i]["account_comment"]=="" && $query[$i]["payment_status"]=="" && $query[$i]["reason"]!="" && $query[$i]["approve_status"]==0){echo "Pending at Accounts";}
        elseif($query[$i]["approve_status"]==0 && $query[$i]["forward_req_user"]!="" && $query[$i]["forward_back_comment"]=="" && $query[$i]["vehicle_status"]==1)   
        {echo "Pending Admin Approval (Req Forward to ".$query[$i]["forward_req_user"].")";}
        elseif($query[$i]["approve_status"]==0 && ($query[$i]["account_comment"]!="" || $query[$i]["payment_status"]!="") && $query[$i]["final_status"]==0 && $query[$i]["vehicle_status"]==1)
        {echo "Pending Admin Approval";}
        elseif($query[$i]["approve_status"]==1 && $query[$i]["vehicle_status"]==1 && $query[$i]["final_status"]!=1){echo "Pending at Tech Support Team";}
        elseif($query[$i]["final_status"]==1){echo "Process Done";}
    } ?></td>
        <td><a href="#" onClick="Show_record(<?php echo $query[$i]["id"];?>,'vehicle_no_change','popup1'); " class="topopup">View Detail</a></td>
      </tr>
      <?php }?>
  </table>
  <div id="toPopup">
