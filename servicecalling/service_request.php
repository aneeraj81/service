...
{
	$query_row = select_query("SELECT * FROM internalsoftware.services WHERE id='".$_GET["rowid"]."'");
	//$rows=mysql_fetch_array($query);
	
	 
	//$date= $query_row[0]["req_date"]; 
	//$account_manager=$query_row[0]["request_by"]; 
	$main_user_id = $query_row[0]["user_id"]; 
	$company = $query_row[0]["company_name"]; 
	$veh_reg = $query_row[0]["veh_reg"]; 
	$city = $query_row[0]['inter_branch'];
	$Device_model = $query_row[0]["device_model"]; 
	$TxtDeviceIMEI = $query_row[0]["device_imei"]; 
	$date_of_install = $query_row[0]["date_of_installation"]; 
	$Notwoking = $query_row[0]["Notwoking"]; 
	$Zone_area = $query_row[0]["Zone_area"];
	$area = select_query("SELECT id,`name` FROM internalsoftware.re_city_spr_1 WHERE id='".$Zone_area."'"); 
	$location = $query_row[0]["location"]; 
	$city = $query_row[0]["inter_branch"];
	$atime = $query_row[0]["atime"]; 
	$atimeto = $query_row[0]["datetimepickerto"]; 
	$pname = $query_row[0]["pname"]; 
	$cnumber = $query_row[0]["cnumber"]; 
	$required = $query_row[0]['required'];
	$IP_Box = $query_row[0]['IP_Box'];
	$atime_status = $query_row[0]['atime_status'];
	
	$datapullingtime = $query_row[0]["datapullingtime"]; 
	$comment = $query_row[0]["comment"]; 
	
	$sql_client = select_query("SELECT Userid AS id,UserName AS sys_username FROM internalsoftware.addclient WHERE Userid='$main_user_id' and Branch_id='".$_SESSION['BranchId']."'");
	//$row = mysql_fetch_array($sql);
	$username = $sql_client[0]['sys_username'];
...
{ 
	...
	$sql_client = select_query("SELECT UserName AS sys_username FROM internalsoftware.addclient  WHERE Userid='$main_user_id'");
	//$row=mysql_fetch_array($sql);
	$username=$sql_client[0]['sys_username'];
...
if($errorMsg=="")	
{
	if($_GET["edit"]==true && $_GET["rowid"]!='')
	{
		if($atime_status=="Till")
		{
			...
			$sql_till = "update `services` set `location`='".$location."', `atime`='".$time."',`atime_status`='".$atime_status."', `pname`='".$pname."', `cnumber`= '".$cnumber."', `device_model`='".$Device_model."', `comment`='".$comment."',`required`='".$required."',`IP_Box`='".$IP_Box."',Zone_area='".$Zone_area."',service_reinstall='".$service_reinstall."',service_status=1,`inter_branch`='".$city."',branch_type='".$branch_type."' where id='".$_GET["rowid"]."'";
			
			
			$execute = select_query($sql_till);			
		...
		if($atime_status=="Between")
		{
			...
			$sql_betwwen = "update `services` set `location`='".$location."', `atime`='".$time."', atimeto='".$totime."',`atime_status`='".$atime_status."', `pname`='".$pname."', `cnumber`= '".$cnumber."', `device_model`='".$Device_model."', `comment`='".$comment."',`required`='".$required."',`IP_Box`='".$IP_Box."',Zone_area='".$Zone_area."',service_reinstall='".$service_reinstall."',service_status=1,`inter_branch`='".$city."',branch_type='".$branch_type."' where id='".$_GET["rowid"]."'";
		
			
			$execute = select_query($sql_betwwen);
			header("location:services.php");
		 }
 ...
 <form name="myForm" action="" onsubmit="return validateForm()" method="post">
   <table style="width: 900px;" cellspacing="2" cellpadding="3" border="1">
		<tr>
            ...
            <td align="center"> 
            <?php
            $main_user_data = select_query("SELECT Userid AS user_id,UserName AS `name` FROM internalsoftware.addclient WHERE Userid='".$main_user_id."'");
            
			$commpany = $masterObj->getCompanyName($main_user_id);
       		
            ?>
           <!-- <input type="hidden" name="main_user_id" id="TxtMainUserId" value="<?php echo $main_user_data[0]['user_id']; ?>" readonly="readonly" />
            <input type="text" name="user_name" id="TxtMainUserId" value="<?php echo $main_user_data[0]['name']; ?>" readonly="readonly" />-->
   ...
   <table style=" padding-left: 100px;width: 600px;" cellspacing="2" cellpadding="3">
       ...
       <tr>
        ...
        <tr>
            ...
            <td><input type="radio" name="service_reinstall"  value="service" id="service" <?php if($query_row[0]['service_reinstall']=='service') {echo "checked=\"checked\""; }?> /> Service 
            <input type="radio" name="service_reinstall" value="re_install" id="service" <?php if($query_row[0]['service_reinstall']=='re_install') {echo "checked=\"checked\""; }?> /> Re-Install </td>
        </tr>
        ...
        <tr>
          ...
          <td>
          <?php $branch_data = select_query("select * from internalsoftware.tbl_city_name where branch_id='".$_SESSION['BranchId']."'");
		  		//$branch_data = mysql_fetch_array($branch);
		  ?>
          <Input type='radio' Name ='inter_branch' id='inter_branch' value= 'Samebranch' <?php if($query_row[0]['branch_type']=='Samebranch'){echo "checked=\"checked\""; }?> onchange="StatusBranch(this.value);"> <?php echo $branch_data[0]["city"];?>
          
          <Input type='radio' Name ='inter_branch' id='inter_branch' value= 'Interbranch' <?php if($query_row[0]['branch_type']=='Interbranch'){echo "checked=\"checked\""; }?>
                    ...
                    <tr>
                        ...
                        <td>
                          <select name="inter_branch_loc" id="inter_branch_loc">
                                ...
                                <?php
                                $city1 = select_query("select * from internalsoftware.tbl_city_name where branch_id!='".$_SESSION['BranchId']."'");
                               for($ct=0;$ct<count($city1);$ct++)
                                {
                                    if($city1[$ct]['branch_id']==$city)
                                    {
                                        $selected="selected";
                                    }
                                    else
                                ...
                                <option value ="<?php echo $city1[$ct]['branch_id'] ?>"  <?echo $selected;?>>
                                <?php echo $city1[$ct]['city']; ?>
                                </option>
                                <?php 
                                } 
                    ...
                    <tr>
                        ...
                        <td>
                          <select name="inter_branch_loc" id="inter_branch_loc">
                                <option value="" >-- Select One --</option>
                                <?php
                                $city1=mysql_query("select * from internalsoftware.tbl_city_name where branch_id!='".$_SESSION['BranchId']."'",$dblink2);
       ...
       // $main_city=mysql_query(" select id,name from re_city_spr_1 where region_code='".$_SESSION['BranchId']."'");
        ...
        <tr>
          ...
          <td>
            ...
            $device_type = select_query("SELECT * FROM internalsoftware.device_type");
            //while ($data=mysql_fetch_assoc($device_type))
			for($dt=0;$dt<count($device_type);$dt++)
            {
                if($device_type[$dt]['device_type']==$Device_model)
                {
                    $selected="selected";
                }
                else
            ...
            <option name="Device_model" value="<?php echo $device_type[$dt]['device_type'] ?>"  <?echo $selected?>  >
            <?php echo $device_type[$dt]['device_type']; ?>
            </option>
            <?php 
            } 
            ...
            <td colspan="2">
                ...
                <table  id="TillTime" align="left" style="width: 240px;display:none;border:1"  cellspacing="5" cellpadding="5">
                        <tr>
                            ...
                            <td>
                                 <input type="text" name="time" id="datetimepicker" value="<?=$query_row[0]['atime']?>" style="width:147px"/>
                ...
                <table  id="BetweenTime" align="left" style="width: 240px;display:none;border:1"  cellspacing="5" cellpadding="5">
                        <tr>
                            ...
                            <td>
                                 <input type="text" name="time1" id="datetimepicker1" value="<?=$query_row[0]['atime']?>" style="width:147px"/>
                        ...
                        <tr>
                            ...
                            <td>
                             <input type="text" name="totime" id="datetimepicker2" value="<?=$query_row[0]['atimeto']?>" style="width:147px"/>
...
?>
<script>StatusBranch12("<?=$query_row[0]['branch_type'];?>");TillBetweenTime12("<?=$query_row[0]['atime_status'];?>");</script>
