...
left join item_master on item_master.item_id =device .device_type  where device_id=".$DeviceId);
$rowDevice = mssql_fetch_array($resultDevice);
mssql_query("update device set device_status=65 where  device_id=".$DeviceId);



mysql_query("INSERT INTO `new_device_addition` (`inst_id`, `date`, `inst_close_date`, `acc_manager`, `client`, `user_id`, `vehicle_no`, `ac`,`immobilizer`,`immobilizer_type`, `device_type`, `device_model`, `device_id`, `device_imei`, `device_sim_num`,installtionRequestID,dimts,sales_manager,inst_name,billing_if_no_reason,billing) VALUES ('".$id."','".$date."','".$rtime."','".$acc_manager."','".$rows['company_name']."',  '".$rows['user_id']."', '".$veh_reg."',  '".$ac."', '".$immobilizer."', '".$immobilizer_type."', 'New',  '".$rowDevice['item_name']."',  '".$rowDevice['itgc_id']."',  '".$rowDevice['device_imei']."',  '".$rowDevice['phone_no']."', '".$rows['id']."','".$rows['dimts']."','".$sales['name']."','".$rows['inst_name']."','".$billing_no_reason."','".$billing."')") or die(mysql_error());
  $errMSG="";
	}
	else
		{
...
if($errMSG=="")
	{  
		...
		mysql_query("update installer set status=0 where inst_id='$inst_id'");
		
		mysql_query("UPDATE installation SET payment_status='$payment_status',amount='$amount',paymode='$paymode',reason='$reason',rtime='$rtime',close_date='".date("Y-m-d")."',inst_date='".date("Y-m-d")."',newpending='0',newstatus='0',device_type='$device_type',billing='$billing',payment='$payment',installation_made='$inst_made',installation_status=5   WHERE id='$id'") or die(mysql_error());	
...
$pending=mysql_query("UPDATE installation SET inst_id='$inst_id', job_type='$job_type' , inst_name='$inst_name',inst_cur_location='$inst_cur_location' ,billing='$billing',payment='$payment' , installation_status=2  WHERE id='$id'");
	//mysql_query("update installer set status=1 where inst_name='$inst_name'");
	//mysql_query("update installer set status=0 where inst_name='$inst_name1'");
...
if(isset($_POST['backservice']))
{
	...
	$pending = mysql_query($update_bcakreason);
...
if(document.form3.rtime.value =="")
 ...
 if(document.form3.device_type.value =="")
   ...
   /*for(K=1;K<=totalcount;K++)
	{
     alert('hiii');
     if(document.form3.Veh_name[K].value =="")
   {
    alert("please enter vehicle name");
    document.form3.Veh_name[K].focus();
   return false;
   }
   if(document.form3.DeviceIMEI[K].value =="")
   {
    alert("please select Device IMEI");
    document.form3.DeviceIMEI[K].focus();
   return false;
   }
   if(document.form3.ac[K].value =="")
   {
    alert("please select AC");
    document.form3.ac[K].focus();
   return false;
   }
   if(document.form3.immobilizer[K].value =="")
   {
    alert("please select Immobilizer");
    document.form3.immobilizer[K].focus();
   return false;
   }
}*/
   /* if(document.form3.Veh_name1.value =="")
   {
    alert("please enter vehicle name");
    document.form3.Veh_name1.focus();
   return false;
   }
   if(document.form3.DeviceIMEI1.value =="")
   {
    alert("please select Device IMEI");
    document.form3.DeviceIMEI1.focus();
   return false;
   }
   if(document.form3.ac1.value =="")
   {
    alert("please select AC");
    document.form3.ac1.focus();
   return false;
   }
   if(document.form3.immobilizer1.value =="")
   {
    alert("please select Immobilizer");
    document.form3.immobilizer1.focus();
   return false;
   }
   if(document.form3.immobilizer1.value =="Y")
    {
		if(document.form3.immobilizer_type1.value =="")
	   {
		alert("please select Immobilizer Type");
...
<td><input type="text" name="name" id="name" readonly value="<?php  echo $sales['name'];?>" /></td>
</tr>
 <tr>
...
<td><input type="text" name="veh_reg" id="veh_reg" readonly value="<?php echo $rows['no_of_vehicals'];?>" /></td>
</tr>
...
<tr>
<td height="32" align="right">Time:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="time" readonly id="time" value="<?php echo $rows['time'];?>" /></td>
</tr>
<tr>
<td height="32" align="right">Vehicle Type :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="veh_type" id="veh_type" readonly value="<?php echo $rows['veh_type'];?>" /></td>
</tr>

<tr>
<td height="32" align="right">Device Model:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="model" readonly id="model" value="<?php echo $rows['model'];?>" /></td>
</tr>
<tr>
<td height="32" align="right">Immobilizer :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="immobilizer_type" id="immobilizer_type" readonly value="<?php echo $rows['immobilizer_type'];?>" /></td>
</tr>
<tr>
<td height="32" align="right">DIMTS :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="dimts" id="dimts" readonly value="<?php echo $rows['dimts'];?>" /></td>
</tr>
<tr>
<td height="32" align="right">Demo:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="demo" id="demo" readonly value="<?php echo $rows['demo'];?>" /></td>
</tr>
<tr>
<td height="32" align="right">Amount:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="payment_req" id="payment_req" readonly value="<?php echo $rows['payment_req'];?>" /></td>
</tr>
<tr>
...
<td><span class="style18">YES 
    <input type="checkbox" name="billing" id="billing" value="Yes" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
     NO 
     <input type="checkbox" name="billing_1" id="billing_1" value="No" /> 
     REASON 
</span>&nbsp;
...
<td height="32" align="right">Time*:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="text" name="rtime" id="rtime" style="width:180px" value="<?php echo $rows['rtime'];?>" /></td>
