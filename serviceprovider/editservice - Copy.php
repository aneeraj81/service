...
{
$id=$_GET['id'];
$query=mysql_query("SELECT * FROM services WHERE id='$id'",$dblink2);
...
{
	$veh_id_get = mysql_query("select id from matrix.services where veh_reg='$veh_reg'",$dblink);
	$veh_reg_id = mysql_fetch_array($veh_id_get);
	$veh_service_id = $veh_reg_id['id'];
		...
		{					 
			...
			if($rows['device_imei']!="")
			{
				...
				if($device_id['device_id'] != "")
				{
				   ...
				   $check_data = mssql_query("select count(device.device_imei) from device join device_repair on device.device_imei=device_repair.device_imei  where device.device_imei='".$rows['device_imei']."' and device.device_status in(84,85,86,79,68,105,62,70,94,95,83,81,66,75,82,57,63) and datediff(dd,device_repair.device_removed_recddate,getdate())>15");
					...
					if($data_count > 0)
				    {
						...
						{
							$inventory_query=mssql_query("update device set device_status=103,dispatch_branch='".$_SESSION['BranchId']."' where device_imei='".$rows['device_imei']."'");
							$inventory_query2 = mssql_query("update device_repair set device_status=103 where current_record=1 and device_imei='".$rows['device_imei']."'");
						}
						else
						{	
							...
							if($device_status == "Permanent")
							{
								...
								mssql_query("update device set device_status=66, is_permanent=1, dispatch_branch='".$_SESSION['BranchId']."' where device_imei='".trim($rows['device_imei'])."'");
							}
							else
							{
								//echo "update device set device_status=66 where device_imei='".trim($rows['device_imei'])."'";
								mssql_query("update device set device_status=66, dispatch_branch='".$_SESSION['BranchId']."' where device_imei='".trim($rows['device_imei'])."'");
							...
							if($total_count>0)
							{
								...
								$inventory_query = mssql_query("insert into device_repair(device_id, client_name,device_imei,veh_no, device_removed_date, device_removed_branch, device_removed_problem, current_record, device_status,Remove_installer_name,veh_service_id) values('".$device_id['device_id']."','".$name."','".$rows['device_imei']."','".$veh_reg."','".date('Y-m-d H:i:s')."','".$_SESSION['BranchId']."','".$reason."','1','66','".$inst_name1."','".$veh_service_id."')");
							   
								// mysql_query("update matrix.services set device_removed_service=1 where id='$veh_service_id'");
							}
							else
							{
								//echo "insert";   
    ...
    else
    {
        ...
        $inventory_query = mssql_query("update device set device_status=65, dispatch_branch='".$_SESSION['BranchId']."' where device_imei='".trim($rows['device_imei'])."'");
		$inventory_query2 = mssql_query("update device_repair set device_status=65 where current_record=1 and device_imei='".trim($rows['device_imei'])."'");
...
if($errorMsg=="")	
{
		...
		if($sim_remove_status == "With Sim")
            {
                ...
                $update_query = mssql_query("update sim set Sim_Status=106, flag=2, active_status=1, branch_id='".$_SESSION['BranchId']."', SimChangeInstallerName='".$inst_name1."', SimRemoveDate='".$sim_remove_date."', SimChangeRemarks='Sim Change - With Sim -".$rows['device_imei']."-".$veh_reg."' where sim_id='".$old_sim_sql_query["sim_id"]."'");
			}
		...
		if($sim_remove_status == "Without Sim")
            {
                ...
                $update_query = mssql_query("update sim set Sim_Status=92, status=0, flag=3, active_status=1,  branch_id='".$_SESSION['BranchId']."', SimChangeInstallerName='".$inst_name1."', SimRemoveDate='".$sim_remove_date."', SimChangeRemarks='Sim Change - Without Sim -".$rows['device_imei']."-".$veh_reg."' where sim_id='".$old_sim_sql_query["sim_id"]."'");
			}
    ...
    if($_POST["device_veh_change"]=="DeviceChange")
    {
			...
			if($data_count > 0)
			{
				...
				$inventory_old_imei_query = mssql_query("update device set device_status=66 where device_imei='".trim($device_imei)."'");
				$inventory_query2 = mssql_query("update device_repair set device_status=66 where current_record=1 and device_imei='".trim($device_imei)."'");
				...
				if($sim_remove_status == "With Sim")
				{
					...
					$update_query = mssql_query("update sim set Sim_Status=106, flag=2, active_status=1, branch_id='".$_SESSION['BranchId']."', SimChangeInstallerName='".$inst_name1."',SimRemoveDate='".$sim_remove_date."', SimChangeRemarks='Device Change - With Sim -".$rows['device_imei']."-".$veh_reg."' where sim_id='".$old_sim_sql_query["sim_id"]."'");
				
					if($total_count>0)
					{
						 mssql_query("update device_repair set current_record=0 where device_imei='".trim($device_imei)."'");
					   
						$inventory_query = mssql_query("insert into device_repair(device_id, client_name,device_imei,veh_no, device_removed_date, device_removed_branch, device_removed_problem, current_record, device_status,Remove_installer_name) values('".$old_imei_device_id['device_id']."','".$name."','".$device_imei."','".$veh_reg."','".date('Y-m-d H:i:s')."','".$_SESSION['BranchId']."','Device Interchange - With Sim -".$rows['device_imei']."','1','66','".$inst_name1."')");
				...
				if($sim_remove_status == "Without Sim")
				{
					...
					$update_query = mssql_query("update sim set Sim_Status=92, status=0, flag=3, active_status=1, branch_id='".$_SESSION['BranchId']."', SimChangeInstallerName='".$inst_name1."', SimRemoveDate='".$sim_remove_date."', SimChangeRemarks='Device Change - Without Sim -".$rows['device_imei']."-".$veh_reg."' where sim_id='".$old_sim_sql_query["sim_id"]."'");
					
					if($total_count>0)
					{
						 mssql_query("update device_repair set current_record=0 where device_imei='".trim($device_imei)."'");
					   
						$inventory_query = mssql_query("insert into device_repair(device_id, client_name,device_imei,veh_no, device_removed_date, device_removed_branch, device_removed_problem, current_record, device_status,Remove_installer_name) values('".$old_imei_device_id['device_id']."','".$name."','".$device_imei."','".$veh_reg."','".date('Y-m-d H:i:s')."','".$_SESSION['BranchId']."','Device Interchange - Without Sim -".$rows['device_imei']."','1','66','".$inst_name1."')");
         ...
         $query="INSERT INTO `device_change` (`date`,acc_manager, `client`, `user_id`, `device_imei`, `mobile_no`, `reg_no`, date_of_install, inst_name, device_model) VALUES ('".$time."','".$acc_manager."','".$client."','".$user_id."','".$device_imei."','".$Devicemobile."','".$veh_reg."','".$date_of_install."','".$inst_name1."','".$rows['device_model']."')";
...
if(isset($_POST['update']))
    ...
    $pending=mysql_query("UPDATE services SET inst_id='$inst_id',inst_name='$inst_name',inst_cur_location='$inst_cur_location' ,billing='$billing',payment='$payment', service_status=2, job_type='$job_type'  WHERE id='$id'",$dblink2);
...
<table style="width: 900px;" cellspacing="2" cellpadding="3" border="1">
		<tr>
            ...
            <td align="center"> 
	            <label><strong><?php echo $rows['name'];?></strong> </label>
            </td>
            
            <td  align="right"><strong>Vehicle No:</strong></td>
            <td align="center">
            	<label><strong><?php echo $rows['veh_reg'];?></strong> </label></td>
                
            <td  align="right"><strong>Notwoking:</strong></td>
            <td align="center"> 
                    <label><strong><?php echo $rows['Notwoking'];?></strong> </label></td>
        </tr>
        <tr>
            <td  align="right"><strong>Location:</label></strong></td>
            <td align="center">
                <label><strong><?php echo $rows['location'];?></strong> </label></td>

            <td  align="right"><strong>Available Time:</label></strong></td>
            <td align="center">
                <label><strong><?php echo $rows['atime'];?></strong> </label></td>
            
            <td  align="right"><strong>Person Name: </strong></td>
            <td align="center">
                <label><strong><?php echo $rows['pname'];?></strong> </label></td>
       </tr>
       <tr>
          <td  align="right"><strong>Contact No:</label></strong></td>
          <td align="center"> <label><strong><?php echo $rows['cnumber'];?></strong> </label></td>
          
          <td  align="right"><strong>Device Model:</label></strong></td>
          <td align="center"> <label><strong><?php echo $rows['device_model'];?></strong> </label></td>
          
          <td  align="right"><strong>Installer Name:</label></strong></td>
          <td align="center"> <label><strong><?php echo $rows['inst_name'];?></strong> </label></td>
       </tr>
   </table>
   
   <table width="50%" border="0" cellpadding="0" cellspacing="0">
    <tr>
        <td><td><input name="id" type="hidden" id="id" value="<?php echo $rows['id'];?>">
                ...
                <input type="hidden" name="installer_id" value="<?php echo $rows['inst_id'];?>" />
                <input type="hidden" name="inst_name1" id="inst_name1" value="<?php echo $rows['inst_name'];?>" />
        </td></td>
    </tr>
    <tr>
    	<td colspan="2">&nbsp;</td>
    ...
    <tr>
        ...
        <td><input type="text" name="Notwoking" id="Notwoking" readonly value="<?php echo $rows['Notwoking'];?>" /></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><input type="text" name="atime" readonly id="atime" value="<?php echo $rows['atime'];?>" /></td>
    </tr>
    <tr>
        <td height="32" align="right">Person Name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><input type="text" name="pname" readonly id="pname" value="<?php echo $rows['pname'];?>" /></td>
    </tr>
    <tr>
        <td height="32" align="right">Contact No:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><input type="text" name="cnumber" readonly id="cnumber" value="<?php echo $rows['cnumber'];?>" /></td>
    </tr>
    <tr>
        <td height="32" align="right">Device Model:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><input type="text" name="device_model" readonly id="device_model" value="<?php echo $rows['device_model'];?>" /></td>
    </tr>

    <tr>
        <td height="32" align="right">Installer Name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
        <td><input type="text" name="inst_name1" readonly id="inst_name1" value="<?php echo $rows['inst_name'];?>" />
        <input type="hidden" name="installer_id" value="<?php echo $rows['inst_id'];?>" /></td>
    </tr>-->
    ...
    <tr>
        ...
        <td><input type="text" name="amount" id="amount" value="<?php echo $rows['amount'];?>" /></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td><input type="text" name="inst_cur_location" id="inst_cur_location" value="<?php echo $rows['inst_cur_location'];?>" /></td>
    </tr>
    <? } ?>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <!--<textarea name="problem" id="problem" style="display:none" value="<?php echo $rows['problem'];?>" /></textarea>-->
        </td>
...
<? if($_GET['show']=="close"){?>
    ...
    $result_reason=mysql_query($query,$dblink2);
