	...
	{ 
	 ...
	 /*if(stristr($reason, '(device ok)') == "")
		...
		if($rows['device_imei']!="")
		{
				...
				{
					echo "This IMEI No does not exist in Inventory.Kindly check IMEI no in Your Inventory Login.";
					die();
				}
		  }
		//}
...
//		}  
	...
	if($_POST["sim_change"]=="SimChange")
	{
		...
		mysql_query($query,$dblink2) or die(mysql_error());
    ...
    if($total_count>0)
    {
     ...
     $inventory_query = mssql_query("insert into device_repair(device_id, client_name,device_imei,veh_no, device_removed_date, device_removed_branch, device_removed_problem, current_record, device_status,Remove_installer_name) values('".$old_imei_device_id['device_id']."','".$name."','".$device_imei."','".$veh_reg."','".date('Y-m-d H:i:s')."','".$_SESSION['BranchId']."','Device Interchange - With Sim','1','66','".$inst_name1."')");
   ...
   if($sim_remove_status == "Without Sim")
    ...
    if($total_count>0)
    {
     ...
     $inventory_query = mssql_query("insert into device_repair(device_id, client_name,device_imei,veh_no, device_removed_date, device_removed_branch, device_removed_problem, current_record, device_status,Remove_installer_name) values('".$old_imei_device_id['device_id']."','".$name."','".$device_imei."','".$veh_reg."','".date('Y-m-d H:i:s')."','".$_SESSION['BranchId']."','Device Interchange - Without Sim','1','66','".$inst_name1."')");
...
<table width="50%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr>
        ...
        <td><input type="text" name="Notwoking" id="Notwoking" value="<?php echo $rows['Notwoking'];?>" /></td>
    </tr>
    <tr>
    ...
    <tr>
        <td>&nbsp;</td>
        <td><input type="checkbox" name="billing" id="billing" value="yes" />Billing 
        <input type="checkbox" name="payment" id="payment" value="yes" />Payment</td>
    </tr>
    <tr>
