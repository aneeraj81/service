...
<div class="table">
  <?php
	...
	$rs = select_query("SELECT * FROM installation  where (installation_status='11' and ((fwd_tech_rm_id is NOT NULL and fwd_reason is NOT NULL) or (fwd_repair_id is NOT NULL or fwd_install_to_repair is NOT NULL))) and (branch_id='".$_SESSION['BranchId']."' or inter_branch='".$_SESSION['BranchId']."') order by id desc");
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
    ...
    ?>
      <tr align="Center"  <? if($rs[$i]['installation_status']==5 or $rs[$i]['installation_status']==6 )  {  ?> style="background:#CCCCCC;" <? }
        ...
        <td width="10%" align="center" style="font-size:12px">&nbsp;<?php echo $rs[$i]['inst_name'];
		if($rs[$i]['job_type']==2)
		{
        ...
        <td><?php if($rs[$i]['fwd_reason'] != "" && $rs[$i]['fwd_tech_rm_id'] != "" && $rs[$i]['fwd_repair_id'] == "") { 
				...
				} else if($rs[$i]['fwd_install_to_repair'] != "" && $rs[$i]['fwd_repair_id'] == "") {
					echo $rs[$i]['fwd_install_to_repair']; 
				} else if($rs[$i]['fwd_install_to_repair'] != "" && $rs[$i]['fwd_repair_id'] != "") {
					echo $rs[$i]['fwd_repair_date'].' - '.$rs[$i]['fwd_install_to_repair']; 
				}?>
        </td>
