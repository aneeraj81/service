    ...
    if($count>0)
        {
            ...
            $update_inventory = mssql_query("update device_replace_ffc set new_device_id='".$new_device_id."', replace_device_imei='".$device_imei."', new_client_name='".$client_name."', new_veh_no='".$new_veh_no."', replaced_date='".date("Y-m-d H:i:s")."', ffc_reason='".$reason.'- Old_Imei-'.$device_imei."', updated_by='".$updated_by."',status='100'  where imei_no='".$NewDevice_imei."' and id='".$ffc_imei_id['id']."'");
        }
        else
        {
           $device_warranty = select_query_live_con("SELECT sys_added_date,TIMESTAMPDIFF( DAY, sys_added_date, NOW()) as age, veh_reg,imei,sys_group_id,name FROM matrix.group_services LEFT JOIN matrix.services ON group_services.sys_service_id=services.id LEFT JOIN matrix.`group` ON `group`.id=group_services.sys_group_id
...
$row=mssql_fetch_array($query);
 ...
 $device_warranty2 = select_query_live_con("SELECT sys_added_date,TIMESTAMPDIFF(MONTH, sys_added_date, NOW()) as age, veh_reg,imei,sys_group_id,NAME FROM matrix.group_services 
 									LEFT JOIN  
									...
									WHERE devices.imei ='".$row['device_imei']."' AND sys_parent_group_id=1 AND sys_group_id!=1998 LIMIT 0,1");
...
//echo $date_Install["month"];
	    ...
	    <tr>
            ...
            <td>
                <input type="text" name="spare_cost" id="spare_cost" value="<?php if($row['spare_cost']!="" && $row['spare_cost']!=0){echo $row['spare_cost'];}?>"/></td>
        </tr>
 ...
 '".$device_warranty2[0]['sys_added_date']."') >=0 AND active_status=1 and device_status='69' and dispatch_branch='".$_SESSION['BranchId']."'");*/
       ...
       $replace_within3=mssql_query("SELECT DISTINCT(device.device_imei),item_name,device.recd_date, DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') AS age
 FROM device left join item_master on item_master.item_id=device.device_type WHERE is_ffc=1 AND DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') < 3 AND
  DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') >=0 AND active_status=1 and device_status='69' and device_type in(select item_id from item_master where parent_id='".$device_model_id['parent_id']."') and dispatch_branch='".$_SESSION['BranchId']."' order by age asc");
 ...
 '".$device_warranty2[0]['sys_added_date']."') >=3 AND active_status=1 and device_status='69' and dispatch_branch='".$_SESSION['BranchId']."'");*/
    ...
    $replace_within6=mssql_query("SELECT DISTINCT(device.device_imei),item_name,device.recd_date, DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') AS age
 FROM device left join item_master on item_master.item_id=device.device_type WHERE is_ffc=1 AND DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') < 6 AND
  DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') >=3 AND active_status=1 and device_status='69' and device_type in(select item_id from item_master where parent_id='".$device_model_id['parent_id']."') and dispatch_branch='".$_SESSION['BranchId']."' order by age asc");
 ...
 '".$device_warranty2[0]['sys_added_date']."') >=6 AND active_status=1 and device_status='69' and dispatch_branch='".$_SESSION['BranchId']."'");*/
    ...
    $replace_within12=mssql_query("SELECT DISTINCT(device.device_imei),item_name,device.recd_date, DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') AS age
 FROM device left join item_master on item_master.item_id=device.device_type WHERE is_ffc=1 AND DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') < 12 AND
  DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') >=6 AND active_status=1 and device_status='69' and device_type in(select item_id from item_master where parent_id='".$device_model_id['parent_id']."') and dispatch_branch='".$_SESSION['BranchId']."' order by age asc");
 ...
 FROM device WHERE is_ffc=1 AND DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') >= 12 AND active_status=1 and device_status='69' and dispatch_branch='".$_SESSION['BranchId']."'");*/
    ...
    $replace_above12=mssql_query("SELECT DISTINCT(device.device_imei),item_name,device.recd_date, DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') AS age
 FROM device left join item_master on item_master.item_id=device.device_type WHERE is_ffc=1 AND DATEDIFF(MONTH,device.recd_date,'".$device_warranty2[0]['sys_added_date']."') >= 12 AND active_status=1 and device_status='69' and device_type in(select item_id from item_master where parent_id='".$device_model_id['parent_id']."') and dispatch_branch='".$_SESSION['BranchId']."' order by age asc");
