...
$data=array();
    ...
    $internal_service_row = select_query("select * from (select * from internalsoftware.services where service_reinstall='service' and device_status!='Permanent' and temp_to_permanent is null and service_status=5 AND reason IN('Device removed (against payment)','Device removed,it was on demo','Demo device removed','Device removed(Buy Back)') and (branch_id='".$_SESSION['BranchId']."' or inter_branch='".$_SESSION['BranchId']."') order by id desc) as services GROUP BY device_imei order by req_date DESC");
        
    $internal_ok = "";
    //while($internal_service_row = mysql_fetch_array($internal_service))
    for($ins=0;$ins<count($internal_service_row);$ins++)
        {
            $internal_ok.= $internal_service_row[$ins]['device_imei']."','";
            
            $arr=array (
                    'id' => $internal_service_row[$ins]["id"],
                    'req_date'=> $internal_service_row[$ins]["req_date"],
                    'request_by' => $internal_service_row[$ins]["request_by"],
                    'company_name' => $internal_service_row[$ins]["company_name"],
                    'inst_id' => $internal_service_row[$ins]["inst_id"],
                    'inst_name' => $internal_service_row[$ins]["inst_name"],
                    'close_date' => $internal_service_row[$ins]["time"],
                    'veh_reg' => $internal_service_row[$ins]["veh_reg"],
                    'imei_no' => $internal_service_row[$ins]["device_imei"],
                    'reason' => $internal_service_row[$ins]["reason"],
                    'temp_to_permanent' => $internal_service_row[$ins]["temp_to_permanent"],                    
                    'temp_permnt_date' => $internal_service_row[$ins]["temp_permnt_date"],
                    'available_time' => $internal_service_row[$ins]["atime"],
                    'name' => $internal_service_row[$ins]["name"],
                    'read_unread' => $internal_service_row[$ins]["read_unread_status"],
                    ) ;
            array_push($data,$arr);
        } 
 ...
 $inventory_imei = select_query_inventory("select device.device_imei, TIMESTAMPDIFF(DAY, device_repair.closecase_date,now()) as pending_days 
                     from inventory.device join inventory.device_repair on device.device_imei=device_repair.device_imei where 
                    device.device_status in (116,62,57,63,82) and dispatch_branch='".$_SESSION['BranchId']."' and 
                    device_repair.current_record=1 and TIMESTAMPDIFF(DAY,device_repair.closecase_date,now())>=30 and device.device_imei!='' 
                    and device.device_imei!='65' order by pending_days desc");
 
    $inventory_ok = "";
    for($tp=0;$tp<count($inventory_imei);$tp++)
    {
        $inventory_ok.= $inventory_imei[$tp]['device_imei']."','";
    }
    $inventory_imei_rslt=substr($inventory_ok,0,strlen($inventory_ok)-3);
    
        
    $inventory_data = select_query("select * from (select * from internalsoftware.services where device_imei in ('".$inventory_imei_rslt."') AND device_imei NOT IN ('".$internal_imei_rslt."') and service_reinstall='service' and temp_to_permanent is null and service_status=5 AND (reason like '%Device removed%' or reason like '%Device Changed%') and (branch_id='".$_SESSION['BranchId']."' or inter_branch='".$_SESSION['BranchId']."') order by id desc) as services GROUP BY device_imei order by req_date DESC");
           
    //while($inventory_data = mysql_fetch_array($queryRes))
    for($qr=0;$qr<count($inventory_data);$qr++)
        {
            
            $arr=array (
                    'id' => $inventory_data[$qr]["id"],
                    'req_date'=> $inventory_data[$qr]["req_date"],
                    'request_by' => $inventory_data[$qr]["request_by"],
                    'company_name' => $inventory_data[$qr]["company_name"],
                    'inst_id' => $inventory_data[$qr]["inst_id"],
                    'inst_name' => $inventory_data[$qr]["inst_name"],
                    'close_date' => $inventory_data[$qr]["time"],
                    'veh_reg' => $inventory_data[$qr]["veh_reg"],
                    'imei_no' => $inventory_data[$qr]["device_imei"],
                    'reason' => $inventory_data[$qr]["reason"],
                    'temp_to_permanent' => $inventory_data[$qr]["temp_to_permanent"],                    
                    'temp_permnt_date' => $inventory_data[$qr]["temp_permnt_date"],
                    'available_time' => $inventory_data[$qr]["atime"],
                    'name' => $inventory_data[$qr]["name"],
                    'read_unread' => $inventory_data[$qr]["read_unread_status"],
                    ) ;
            array_push($data,$arr);
        } 
