...
if($_POST["submit"])
{
    ...
    if($selecttype=="" or $selecttype==0)
    {
        ...
        where current_record=1 and device_repair.device_imei='".$value."'");
    ...
    elseif($selecttype==1)
    {
        ...
        device_removed_problem, problem,actual_problem, spare_cost, device.device_status, device_repair.device_removed_date, 
        device_repair.ManufactureRemarks from inventory.device left join inventory.device_repair on device.device_id=device_repair.device_id 
        where  current_record=1 and device_repair.client_name LIKE '%".$value."%'");
    }
    else
    {
        $rs = select_query_inventory("select id, device_repair.client_name , veh_no, device.device_imei, opencase_date, closecase_date, 
        device_removed_problem, problem,actual_problem, spare_cost, device.device_status, device_repair.device_removed_date, 
        device_repair.ManufactureRemarks,manufacturesenddate from device left join device_repair on device.device_id = device_repair.device_id 
        where current_record=1 and device.device_status in(84,85,86)");
    }
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
    ...
    ?>
      <tr align="Center"  <?=$Bgcolor;?> > 
        ...
        <?php if($selecttype == 2){?>
        <td>&nbsp;<?php echo  $rs[$i]['manufacturesenddate'] ?></td>
        <?php } ?>
        <td>&nbsp;<?php echo $rs[$i]['spare_cost'] ;?></td>
        <td>&nbsp;
          <?php  $days = $device_warranty[0]['age'] ; 
            if($days < 12 && $days >=0 && $device_warranty[0]['sys_added_date']!=""){ echo "Warranty" ;} else{ echo "Out of Warranty";}?></td>
        <td>&nbsp;
          <?php if($rs[$i]['device_status']==20){ echo "Raw Inventory" ;} if($rs[$i]['device_status']==55){ echo "Installed" ;} 
        ...
        if($rs[$i]['device_status']==67){ echo "Device Removed Recd" ;} if($rs[$i]['device_status']==68){ echo "Open Repair Case" ;}
        if($rs[$i]['device_status']==69){ echo "FFC Done But Pending for Replace as New/Old" ;} 
        if($rs[$i]['device_status']==70){ echo "Dead Device" ;}
        ...
        if($rs[$i]['device_status']==105){ echo "Device Has been Repaired But Not Received By Stock" ;} 
        ...
        if($rs[$i]['device_status']==116){ echo "Device Has been Repair and Deposist to Stock Delhi" ;}
        if($rs[$i]['device_status']==122){ echo "Device Has been Send But Not Received By R and D" ;}
     ?></td>
