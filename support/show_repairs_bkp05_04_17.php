...
{
	$rs = mssql_query("select id,device_repair.client_name ,veh_no,device.device_imei,opencase_date,closecase_date,device_removed_problem, problem,actual_problem, spare_cost, device.device_status,device_repair.device_removed_date,device_repair.ManufactureRemarks from device left join device_repair on device.device_id=device_repair.device_id where current_record=1 and device_repair.device_imei='".$value."'");
	
}
elseif($selecttype==1)
{
	$rs = mssql_query("select id,device_repair.client_name ,veh_no,device.device_imei,opencase_date,closecase_date,device_removed_problem, problem,actual_problem, spare_cost, device.device_status,device_repair.device_removed_date,device_repair.ManufactureRemarks from device left join device_repair on device.device_id=device_repair.device_id where  current_record=1 and device_repair.client_name LIKE '%".$value."%'");
}
else
{
	$rs = mssql_query("select id,device_repair.client_name ,veh_no,device.device_imei,opencase_date,closecase_date,device_removed_problem, problem,actual_problem, spare_cost, device.device_status, device_repair.device_removed_date,device_repair.ManufactureRemarks,manufacturesenddate from device left join device_repair on device.device_id = device_repair.device_id where current_record=1 and device.device_status in(84,85,86)");
}
//$mode=$_GET['mode'];
//if($mode=='') { $mode="new"; }
...
// CloseCaseForRepairedDevice = 69
// dead=70

  /*if($mode=='close')
	{*/
	 
	/*$rs = mssql_query("select id,device_repair.client_name ,veh_no ,device_repair.device_imei ,opencase_date ,closecase_date, actual_problem, spare_cost, device.device_status, device_repair.device_removed_date from device_repair left join device on device_repair.device_id=device.device_id where device.device_status=69");*/
	
 	/*$rs = mssql_query("select id,device_repair.client_name ,veh_no ,device_repair.device_imei ,opencase_date ,closecase_date,device_removed_problem, problem, actual_problem, spare_cost, device.device_status, device_repair.device_removed_date from device_repair left join device on device_repair.device_id=device.device_id  where device.device_status=69");
	
	 $Bgcolor='style="background:#CCCCCC"';
	}
	else if($mode=='new')
	{*/
	 
	/*$rs = mssql_query("select id,device_repair.client_name ,veh_no ,device_repair.device_imei ,opencase_date ,
closecase_date,actual_problem,spare_cost,device.device_status,device_repair.device_removed_date
 ...
 where device.device_status in (67,68)");*/
 
 	/*$rs = mssql_query("select id,device_repair.client_name ,veh_no,device_repair.device_imei,opencase_date,closecase_date,device_removed_problem, problem,actual_problem, spare_cost, device.device_status,device_repair.device_removed_date from device left join device_repair on device.device_id=device_repair.device_id where device.device_status in (67,68)");
	
	
	$Bgcolor='style="background:#F6F"';
	}	

	else if($mode=='dead')
	{*/
	 
	/*$rs = mssql_query("select id,device_repair.client_name ,veh_no ,device_repair.device_imei ,opencase_date ,closecase_date, actual_problem, spare_cost, device.device_status, device_repair.device_removed_date from device_repair left join device on device_repair.device_id=device.device_id where device.device_status=70");*/
...
<option id="2" value="2" <?php if($selecttype==2){ ?> selected="selected" <?php } ?>>Manufacture </option>
 <?php } ?>
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
	<thead> 
      ...
      <th><font color="#0E2C3C"><b>Client Name </b></font></th>
        ...
        <?php if($selecttype == 2){?>
        <th><font color="#0E2C3C"><b>Manufacture Send Date</b></font></th>
        <?php } ?>
	...
	$device_warranty = select_query_live_con("select group_services.sys_added_date,TIMESTAMPDIFF( month, sys_added_date, NOW()) as age from matrix.services left join matrix.group_services on services.id=group_services.sys_service_id where veh_reg='".$row['veh_no']."' limit 1");
    ...
    ?>  
 	...
 	<tr align="Center"  <?=$Bgcolor;?> >
        ...
        <?php if($selecttype == 2){?>
        <td>&nbsp;<?php echo  $row['manufacturesenddate'] ?></td>
        <?php } ?>
		<td>&nbsp;<?php echo $row['spare_cost'] ;?></td>
        <td>&nbsp;<?php  $days = $device_warranty[0]['age'] ; 
				if($days < 12 && $days >=0 && $device_warranty[0]['sys_added_date']!=""){ echo "Warranty" ;} else{ echo "Out of Warranty";}?></td>
       <td>&nbsp;<?php if($row['device_status']==20){ echo "Raw Inventory" ;} if($row['device_status']==55){ echo "Installed" ;} 
        ...
        if($row['device_status']==67){ echo "Device Removed Recd" ;} if($row['device_status']==68){ echo "Open Repair Case" ;}
        if($row['device_status']==69){ echo "FFC Done But Pending for Replace as New/Old" ;} if($row['device_status']==70){ echo "Dead Device" ;}
        ...
        if($row['device_status']==105){ echo "Device Has been Repaired But Not Received By Stock" ;} 
        if($row['device_status']==106){ echo "Sim is Pending for Deactivation" ;} if($row['device_status']==108){ echo "Archiev Dead List" ;}
        if($row['device_status']==109){ echo "Send To Repair Centre Manufacture" ;} 
        if($row['device_status']==120){ echo "Missing Device In Inventory" ;}
        if($row['device_status']==116){ echo "Device Has been Repair and Deposist to Stock Delhi" ;}
        if($row['device_status']==122){ echo "Device Has been Send But Not Received By R and D" ;}
     ?></td>
