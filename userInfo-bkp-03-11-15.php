...
for($N=1;$N<=$_GET["RowId"];$N++)
    ...
    $deactive_query = mysql_query("SELECT sys_service_id FROM matrix.group_services WHERE active=0 AND sys_group_id=(SELECT sys_group_id FROM matrix.group_users where sys_user_id='".$UserId."')",$dblink);
...
$msg.=" <select  name='DeviceIMEI$N' required>
  ...
  while($row = mysql_fetch_array($result))
  {
     ...
     $device_imei = str_replace("_","",$row['imei']);
    if($device_imei !='')
    {
...
if(isset($_GET['action']) && $_GET['action']=='DetailClient')
{
   ...
   if($branch_id != "")
   {
       $query = mysql_query("select * from installer where branch_id='".$branch_id."' and is_delete=1 order by inst_name asc");
       
       $main_user_data = mysql_query("SELECT Userid AS user_id,UserName AS name FROM addclient WHERE Branch_id='".$branch_id."'");
        ...
        $msg.=" <table><tr><td >Installer Name</td><td>
         ...
         while($data=mysql_fetch_array($query))  
         {
          ...
          $msg .="<option value=".$data['inst_id'].">".$data['inst_name']."</option>";
         ...
         while ($data2=mysql_fetch_assoc($main_user_data))
           {
               
          $msg .="<option value=".$data2['user_id'].">".$data2['name']."</option>";
...
if(isset($_GET['action']) && $_GET['action']=='Instaltiondate')
    ...
    if(isset($_GET['action']) && $_GET['action']=='Notwokingdate')
    {
        ...
        echo date("d-M-Y H:i:s",strtotime($row[0]["notworkingDate"]));
    }
...
<tr><td>No. Of Vehicles:   </td><td><?echo $row[0]["no_of_vehicals"];?></td></tr>
...
<tr><td>Device Type:  </td><td><?echo $row[0]["device_type"];?></td>
 </tr>
...
<tr><td>Forward to Repair :</td><td><?echo $row[0]["fwd_install_to_repair"];?></td></tr>
<tr><td>Repair Comment:</td><td><?echo $row[0]["fwd_repair_to_install"];?></td></tr>
...
<?php if($_SESSION['BranchId']==1){?>
    ...
    <tr><td>Admin Approval</td>  <td><?if($row[0]["approve_status"]==1) echo "Approved";?></td></tr>
...
$data=select_query_live("select * from matrix.comment where service_id='".$RowId."' order by date desc");
...
for($d=0;$d<count($data);$d++)
    {
        $ffc_device = $data[$d]["is_ffc"];
        $repaired_device = $data[$d]["is_repaired"];
 ...
 echo '<tr ><td>'.$data[$d]["device_imei"]. '</td><td>'.$data[$d]["item_name"].'</td><td>'.$device_status.'</td></tr>';
    }
    echo '</table>';
...
<tr><td>Specialist</td><td><?echo $row[0]["specialist"];?></td></tr>
<tr><td>Tool Kit</td><td><?echo $row[0]["toolkit"];?></td></tr>
<tr><td>Work Status</td><td><?echo $row[0]["work_status"];?></td></tr>
...
<tr><td><strong>Req Forwarded to</strong></td><td><?echo $row[0]["forward_req_user"];?></td></tr>
<tr><td><strong>Forward Comment</strong></td><td><?echo $row[0]["forward_comment"];?></td></tr>
<tr><td><strong>F/W Request Back Comment</strong></td><td><?echo $row[0]["forward_back_comment"];?></td></tr>

<tr><td><strong>Approval Date</strong></td><td><?
if($row[0]["approve_status"]==1 && $row[0]["approve_date"]!='')
{
echo date("d-M-Y h:i:s A",strtotime($row[0]["approve_date"]));
}
...
?></td>
    ...
    <? }
    else If($tablename=="deactivation_of_account")
        {
         $query = "SELECT * FROM ".$tablename." where id=".$RowId;
            $row=select_query($query);
         $query1 = mysql_query("SELECT imei_of_removed_devices,other_imei_removed,client,device_location FROM stock_deactivation_of_account where deactivate_acc_id=".$RowId,$dblink2);
...
?></td>
    ...
    <? }
        ...
        $query = "SELECT * FROM ".$tablename." where id=".$RowId;
            $row=select_query($query);
        $query1 = mysql_query("SELECT imei_of_removel_devices,other_imei_removed,client,device_location FROM stock_del_form_debtors where del_debtors_id=".$RowId,$dblink2);
